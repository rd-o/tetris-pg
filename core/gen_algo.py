...
def add_piece_to_area(piece_shape, piece_x, piece_y, area):
    for y, row in enumerate(piece_shape):
        for x, cell in enumerate(row):
            if cell:
                area[piece_y + y][piece_x + x] = cell


def get_score(tetris, model, s_lines, neat=False):
    print(tetris.simulated_piece)
    area = np.asarray(tetris.grid)
    add_piece_to_area(tetris.simulated_piece.shape, tetris.simulated_piece.x, tetris.simulated_piece.y, area)
